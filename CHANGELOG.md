...
- `profile` no longer inflates its samples-in-flight count to at least `threads/3 + 1`. The floor could only exceed `min(num_files, threads)` when it also exceeded the number of files, which `get_chunks` caps at anyway, so it was inert here; upstream dropped it because it starved their per-file sketch pipeline.
